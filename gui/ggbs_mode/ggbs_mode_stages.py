...
class StageOne(tk.Frame):
    ...
    def __init__(self, master_si, analyzer, controller):
        ...
        self.entry_list = [self.x_strength, self.days, self.defective_rate, self.ggbs_proportion,
                           self.sd, self.spec_k, self.s_margin]
        self.set_active()
    ...
    def display_fimages(self):
        ...
        """
        # Display required entry texts
        req_fimages = load_tk_image(optimix_paths.ggbs_assets['required_entries_ggbs'])
        self.stage_canvas.req_fimages = req_fimages
        ...
        # Display optional entry texts
        optional_fimages = load_tk_image(optimix_paths.aem_assets['opt_entries_image'])
        self.stage_canvas.optional_fimages = optional_fimages
        self.stage_canvas.create_image(
            595.2250061035156,
            440.05750274658203,
            image=optional_fimages
        )

        # Little painting mod here lol
        self.stage_canvas.create_rectangle(400, 450, 850, 880,
                                           fill=BG, outline=BG)
        self.stage_canvas.create_rectangle(600, 280, 1095, 340,
                                           fill=BG, outline=BG)
    ...
    def display_entries(self):
        ...
        # Entry 4 (ggbs proportion)
        # Return previously saved values
        ggbs_prop_value = tk.StringVar(self.stage_canvas)
        ggbs_prop_value.set(self.analyzer.data['Specified variables']['ggbs Proportion'])

        self.ggbs_proportion = tk.Entry(
            self.stage_canvas,
            textvariable=ggbs_prop_value,
            background=BG,
            foreground=white_color,
            font=lato,
            highlightthickness=1
        )
        self.ggbs_proportion.place(
            x=385.0,
            y=300.0,
            width=77.0,
            height=28.0
        ...
        # Entry 6 (Custom Standard Deviation)
        # Return previously saved values
    ...
    def collect_data(self):
        ...
        # GGBS Proportion
        self.analyzer.sync_input(self.ggbs_proportion, 'Specified variables', 'ggbs Proportion')
    ...
    def special_check(self):
        ...
        self.analyzer.special_check(mode='GGBS')
    ...
    def check_fill_status(self):
        ...
        for entry in [self.x_strength, self.days, self.ggbs_proportion]:
        ...
        for entry in [text_box for text_box in self.entry_list if text_box not in [self.x_strength, self.days, self.ggbs_proportion]]:
            value = entry.get().strip()
    ...
    def calculate(self):
        ...
        self.analyzer.calculate_margin(mode='GGBS')
        self.analyzer.calculate_target_mean_strength(mode='GGBS')
        self.analyzer.calculate_approx_strength(mode='GGBS')
        self.analyzer.calculate_fwc_ratio(mode='GGBS')
...
class StageTwo(tk.Frame):
    ...
    def __init__(self, master_sii, analyzer, controller):
        ...
        # Make a list of all entries, color change and perform numerical check
        self.entry_list = [self.water_reduc]

        # Make selected entry active
        self.set_active()
    ...
    def display_fimages(self):
        ...
        instruction_ii = load_tk_image(optimix_paths.pfa_assets['stage_ii_instructions_pfa'])
        ...
        water_reduc = load_tk_image(optimix_paths.ggbs_assets['wr_image'])
        self.stage_canvas.wr_image = water_reduc
        self.stage_canvas.create_image(
            320.7124938964844,
            390.0574951171875,
            image=water_reduc
        ...
        # 'kg/m3'
        self.stage_canvas.create_text(525, 387, font=font.Font(family=calibri, size=11),
                                      fill=white_color, activefill=white_color, text='kg/m\u00b3')
    ...
    def display_entries(self):
        ...
        # Entry 1 (Free water content reduction)
        # Return previously saved values
        wr_value = tk.StringVar(self.stage_canvas)
        wr_value.set(self.analyzer.data['Additional info']['Water Content Reduction'])

        # Display entry 1
        self.water_reduc = tk.Entry(
            self.stage_canvas,
            textvariable=wr_value,
            background=BG,
            foreground=white_color,
            font=lato,
            highlightthickness=1
        )
        self.water_reduc.place(
            x=410.0,
            y=376,
            width=77.0,
            height=28.0
    ...
    def collect_data(self):
        ...
        self.analyzer.sync_input(self.water_reduc, 'Additional info', 'Water Content Reduction')
    ...
    def check_fill_status(self):
        ...
        fill_status = []
        messagebox_has_been_shown = False
        wr_val = self.water_reduc.get().strip()
        ...
        if all(element == 0 for element in fill_status):
        ...
        elif wr_val and not wr_val.replace('.', '').isdigit():
            ...
            if messagebox_has_been_shown:
                ...
                messagebox.showerror(
                    title="Invalid input",
                    message="Please input a valid value for the free water content reduction.",
                    parent=self.stage_canvas
                )
                ...
                # Clear the entry
                self.water_reduc.delete(0, tk.END)
    ...
    def calculate(self):
        ...
        Also given that the compulsory entries have been filled
        """
        self.analyzer.calculate_fw_content(mode='GGBS')
...
class StageThree(tk.Frame):
    ...
    def __init__(self, master_siii, analyzer, controller):
        ...
        # Make a list of all entries, color change and perform numerical check
        self.entry_list = [self.max_c, self.min_c, self.max_fc_ratio]
        self.set_active()
    ...
    def display_fimages(self):
        ...
        max_fc_ratio = load_tk_image(optimix_paths.pfa_assets['max_cr'])
        self.stage_canvas.max_cr = max_fc_ratio
        self.stage_canvas.create_image(
            344.7124938964844,
            274,
            image=max_fc_ratio
        ...
        limit_note = load_tk_image(optimix_paths.ggbs_assets['lw_ggbs'])
        self.stage_canvas.ln = limit_note
        self.stage_canvas.create_image(
            550,
            350,
            image=limit_note
    ...
    def collect_data(self):
        ...
        self.analyzer.sync_input(self.max_fc_ratio, 'Specified variables', 'Maximum free water-cement ratio')
    ...
    def calculate(self):
        ...
        Also given that the compulsory entries have been filled
        """
        self.analyzer.calculate_cement_content(mode='GGBS')
...
class StageFour(tk.Frame):
    ...
    def check_fill_status(self):
        ...
        if self.analyzer.ssd_value_error or self.analyzer.invalid_ssd:
            ...
            self.analyzer.compute_wet_conc_density(mode='PFA')
            self.analyzer.override_density()
            self.analyzer.compute_total_agg_content(mode='PFA')
    ...
    def calculate(self):
        ...
        self.analyzer.compute_wet_conc_density(mode='GGBS')
        self.analyzer.override_density()
        self.analyzer.compute_total_agg_content(mode='GGBS')
...
class StageFive(tk.Frame):
    ...
    def check_fill_status(self):
        ...
        # Perform background calculations before proceeding to check
        self.collect_data()
        self.analyzer.compute_fine_agg_proportion(mode='GGBS')
        self.analyzer.compute_agg_content(mode='GGBS')
    ...
    def calculate(self):
        ...
        self.analyzer.oven_dry_batching()
        self.analyzer.proportion_coarse_agg()
        self.analyzer.summarize_results(mode='GGBS')
