"""
This module displays mix design result for every stage in the cement.ggbs
mix design mode.
...
class StageOneResults(tk.Toplevel):
    ...
    def __init__(self, analyzer, controller, stage):
        ...
        # Window settings
        self.title(
            "OptiMix: Stage One Design Results (Portland Cement/Ground Granulated Blast-furnace Slag Concrete Mix)")  # window title
    ...
    def display_header(self):
        ...
        # Display text 'Portland Cement/Ground Granulated Blast-Furnace Slag Concrete Mix'
        self.base_canvas.create_text(628,
                                     102,
                                     fill=white_color,
                                     font=font.Font(family=calibri, size=13, weight='bold'),
                                     text=f"(Portland Cement/ggbs Concrete Mix)")
    ...
    def display_target_wc_ratio(self, status: dict):
        ...
        if status['Margin'] == 'To be Determined':
            if status['k'] == 'To be Determined':
                ...
                fm_val = np.round(float(self.analyzer.calc_data['fm']), 2)
                fwc_ratio_intro = f"To determine the free-water/cement-ggbs ratio, the approximate strength of {approx} N/mm\u00b2 \n" \
                ...
                final_wc_message = f"The required free-water/cement ratio of this portland cement/ggbs mix is {np.round(self.analyzer.calc_data['initial_fwc_ratio'], 2)}."
                y_coord = 2525
                ...
                # Place the visualization
                fig_iv_plot = Iself.analyzer.fig_iv_png_image
                self.base_canvas.fig_iv = fig_iv_plot
                self.base_canvas.create_image(
                    395.00000762939453,
                    2065.99998474121094,
...
class StageTwoResults(tk.Toplevel):
    ...
    def __init__(self, analyzer, controller, stage):
        ...
        # Window settings
        self.title(
            "OptiMix: Stage Two Design Results (Portland Cement/Ground Granulated Blast-furnace Slag Concrete Mix)")  # window title
        self.iconbitmap(optimix_paths.icons['icon_16'])  # window icon
        self.resizable(False, True)
    ...
    def display_fw_content(self, status: dict):
        ...
        if status['fw_content'] == 'Different Agg Types':
            ...
            rec_i = load_tk_image(optimix_paths.doe_assets['report_rectangles'][3])
            ...
            self.base_canvas.tab_iii = tab_iii
            self.base_canvas.create_image(
                387.00000762939453,
                430.99998474121094,
                image=tab_iii
            ...
            # Display text
            text = f"For a portland cement-ggbs concrete mix featuring a maximum aggregate size of {max_agg_size}mm, \n" \
                   ...
                   f"{vocab[slump_value][1]}, the required water content from the table above is estimated below: \n" \
                   ...
                   f"With a specified water content reduction of {np.round(reduction, 1)} kg/m\u00b3,\n" \
                   f"The required free-water content for the cement-ggbs mix is {np.round(w, 1)} kg/m\u00b3."
            ...
            text = f"For a portland cement-ggbs concrete mix featuring a maximum aggregate size of {max_agg_size}mm, \n" \
                   ...
                   f"the free-water content from the table above is {initial_fw} kg/m\u00b3.\n" \
                   f"\n" \
                   f"With a specified water content reduction of {np.round(reduction, 1)} kg/m\u00b3,\n" \
                   f"the required free-water content for the cement-ggbs mix is {np.round(w, 1)} kg/m\u00b3."
...
class StageThreeResults(tk.Toplevel):
    ...
    Methods:
        ...
        - display_ggbs_content(): Displays the calculations regarding the determination of ggbs content
        - display_specified_limits(status): Displays the calculations regarding cement content checks
        - display_wcg_limits(status): Displays W/C+G limit checks based on the user's specifications.
        - display_results(): Displays all results
        - fix_boundary(): fix scroll boundary in the TopLevel window
    """
    ...
    def __init__(self, analyzer, controller, stage):
        ...
        # Window settings
        self.title(
            "OptiMix: Stage Three Design Results (Portland Cement/Ground Granulated Blast-furnace Slag Concrete Mix)")  # window title
        self.iconbitmap(optimix_paths.icons['icon_16'])  # window icon
        self.resizable(False, True)
    ...
    def display_cement_content_calculation(self):
        ...
        p = np.round(float(self.analyzer.data['Specified variables']['ggbs Proportion']), 2)
        hundred_minus_p = 100 - p
        cement_content = np.round(float(self.analyzer.calc_data['calc_cement_content']), 2)

        text = f"Given a free-water content (W) of {fw_content}kg/m\u00b3 & a predetermined W / (C + G) of {initial_fwc_ratio}\n" \
               f"The combined cementitious content = {fw_content}kg/m\u00b3 / {initial_fwc_ratio} = {cement_content} kg/m\u00b3.\n" \
               f"With a specified ggbs proportion of {p}%, the corresponding proportion of \n" \
               f"portland cement will be {hundred_minus_p}% of the total cementitious content. \n" \
    ...
    def display_ggbs_content(self):
        """
        Display the calculations regarding the determination of ggbs content.
        - Background tab
        - Title
        - The calculation of the ggbs content
        """
        # Display the background tab
        ...
        # Show the tab's title, "Determination of ggbs content"
        self.base_canvas.create_text(225.3,
                                     430,
                                     fill=white_color,
                                     activefill=np.random.choice(report_title_colors),
                                     font=font.Font(family=calibri, size=17, weight='bold'),
                                     text="Determination of ggbs Content")

        # Display ggbs content calculation
        cement_content = np.round(float(self.analyzer.calc_data['calc_cement_content']), 2)
        p = np.round(float(self.analyzer.data['Specified variables']['ggbs Proportion']), 2)
        g = np.round(float(self.analyzer.calc_data["init_G"]), 2)

        text = f"With a ggbs proportion of {p}% and a combined cementitious content of {cement_content}kg/m\u00b3, \n" \
               f"The required ggbs content in the cement-ggbs concrete mix is {g} kg/m\u00b3"
    ...
    def display_specified_limits(self, status: dict):
        ...
        c = np.round(float(self.analyzer.calc_data['init_C']), 2)
        f = np.round(float(self.analyzer.calc_data['init_G']), 2)
        total_cement = np.round(float(self.analyzer.calc_data['calc_cement_content']), 2)

        tc = f"The combined portland cement and ggbs content in the mix is expressed below: \n" \
        ...
        if status["max_content"] == 'Specified' and status["min_content"] == 'Specified':
            ...
            if status["cement_content"] == "Within Range":
                ...
                text = f"The combined cement content, {calc_content}kg/m\u00b3, falls within the specified maximum\n" \
                       f"and minimum limits, making it the required cement+ggbs content in the concrete mix."
                ...
                _f = np.round(float(self.analyzer.calc_data['init_ii_G']), 2)
                ...
                text = f"The combined cementitious content is below the specified minimum of {min_content}kg/m\u00b3.\n" \
                       ...
                       f"and {_f}kg/m\u00b3 of ggbs, ensuring a total cementitious content of {min_content}kg/m\u00b3."
            ...
            if status["cement_content"] == "Below Minimum":
                ...
                text = f"The combined cement+ggbs content, {calc_content}kg/m\u00b3, is more than the specified minimum, \n" \
                       f"making it the required cementitious content for the Portland cement + ggbs concrete mix."
            ...
            if status["cement_content"] == "Within Range":
                ...
                text = f"The combined cement content, {calc_content}kg/m\u00b3, is lower than the specified maximum, \n" \
                       f"making it the required cementitious content of the Portland cement/ggbs concrete mix."
    ...
    def display_wcg_limits(self, status: dict):
        """
        ...
        # Display wcg ratio calculation
        w = np.round(float(self.analyzer.calc_data['fw_content']), 2)
        c_f = np.round(float(self.analyzer.calc_data['cement_content']), 2)
        wcg = np.round(float(self.analyzer.calc_data['calc_wcg_ratio']), 2)

        wcg_text = f"With a free-water content of {w}kg/m\u00b3 and a total cementitious content of {c_f}kg/m\u00b3, \n" \
                   f"The required (W / C + G) ratio = {w}kg/m\u00b3 / {c_f}kg/m\u00b3 = {wcg}."
        ...
        if status['limiting wcg'] == 'Specified':
            # Display the maximum free-water to cement ratio
            max_wcg = self.analyzer.data['Specified variables']['Maximum free water-cement ratio']
            max_wcg_text = f"The specified maximum free water-cement ratio is {max_wcg}."
            self.base_canvas.create_text(76,
                                         960,
                                         fill=white_color,
                                         anchor='nw',
            ...
            if status['wcg_status'] == 'More than Maximum':
                ...
                adj_f = np.round(float(self.analyzer.calc_data["G"]), 2)

                text = f"The calculated (W / C + G) exceeds the specified maximum. To meet this requirement, \n" \
                       f"the mix is adjusted to incorporate a total cementitious content of {adj_cement_content}kg/m\u00b3,\n" \
                       f"{adj_c}kg/m\u00b3 of Portland cement and {adj_f}kg/m\u00b3 of ggbs.\n"
            ...
            elif status['wcg_status'] == 'Less than Maximum':
                ...
                _f = np.round(float(self.analyzer.calc_data["G"]), 2)

                text = f"The calculated (W / C + G) is lower than the specified maximum. \n" \
                       f"No further changes will be made. The total cementitious content in the mix is {cement_content}kg/m\u00b3. \n" \
                       f"The required Portland cement content is {_c}kg/m\u00b3. The ggbs content is {_f} kg/m\u00b3. "
        ...
        elif status['limiting wcg'] == 'Unspecified':
            cement_content = np.round(float(self.analyzer.calc_data['final_cementitious_content']), 2)
            _c = np.round(float(self.analyzer.calc_data["C"]), 2)
            _f = np.round(float(self.analyzer.calc_data["G"]), 2)
    ...
    def display_results(self):
        """
        Displays the determination of portland cement content, ggbs content, total (cement + ggbs) comparison, and w/c+f comparison.
        Cases:
        ...
        # Display ggbs content calculations
        self.display_ggbs_content()

        # Requirements check
        self.display_specified_limits(status=current_status)
        self.display_wcg_limits(status=current_status)
...
class StageFourResults(tk.Toplevel):
    ...
    def __init__(self, analyzer, controller, stage):
        ...
        # Window settings
        self.title(
            "OptiMix: Stage Four Design Results (Portland Cement/Ground Granulated Blast-furnace Slag Concrete Mix)")  # window title
        self.iconbitmap(optimix_paths.icons['icon_16'])  # window icon
        self.resizable(False, True)
    ...
    def display_total_agg_content(self):
        ...
        if self.analyzer.specified_conc_density:
            ...
            text = f"The total cementitious content ({c} kg/m\u00b3) and the free-water content\n" \
                   ...
                   f"The total aggregate content of the Portland cement/ggbs concrete mix is {tac}kg/m\u00b3. \n"
            ...
            if self.analyzer.tac_content['ssd_value'] == 'unspecified':
                if self.analyzer.tac_content['Agg_status'] == 'Same types, crushed':
                    # Plot figure 5
                    self.analyzer.plot_figure_v(status='crushed or uncrushed, unspecified', mode='GGBS')
                    ...
                    # Plot figure 5
                    self.analyzer.plot_figure_v(status='crushed and uncrushed, unspecified', mode='GGBS')
                ...
                # Plot figure 5
                self.analyzer.plot_figure_v(status='specified', mode='GGBS')
...
class StageFiveResults(tk.Toplevel):
    ...
    Methods:
        ...
        - display_ggbs_quantity(report_no, canvas): Displays the 'ggbs _____ kg'
    ...
    def __init__(self, analyzer, controller, stage):
        ...
        # Window settings
        self.title(
            "OptiMix: Stage Five Design Results (Portland Cement/Ground Granulated Blast-furnace Slag Concrete Mix)")  # window title
        self.iconbitmap(optimix_paths.icons['icon_16'])  # window icon
        self.resizable(False, True)
    ...
    def display_recommended_fineagg_prop(self, canvas):
        ...
        text = f"For a maximum aggregate size of {max_agg_size}mm, a slump range of {vocab[slump_value][0]} \n" \
               ...
               f"fine aggregate in the Portland cement/ggbs concrete mix is determined using the figure below:"
    ...
    def roundoff_to_nearest_five_kg(self, report_no: str, canvas):
        ...
        if report_no == 'a':
            ...
            # Round off the ggbs content quantity
            ggbs_new_txt = f'{self.analyzer.design_results["ggbs"][self.result_accuracy_switch]}'
            canvas.itemconfigure(tagOrId='ggbs_qty', text=ggbs_new_txt)
    ...
    def display_ggbs_quantity(self, report_no: str, canvas):
        """
        Displays the 'ggbs _____ kg'
        :param canvas: (tk.Canvas): the target canvas for the top level widget
        :param report_no: There are two final mix design reports for now, "a" & "b"
        """
        if report_no == 'a':
            # Load the image
            ggbs_a = load_tk_image(optimix_paths.ggbs_assets['ggbs_report_a'])
            canvas.ggbs_a = ggbs_a

            # Display 'ggbs ____ kg'
            canvas.create_image(
                227.60000762939453,
                497.33331804549453,
                image=ggbs_a
    ...
    def display_txt_quantities(self, report_no: str, canvas, mix_design_results):
        ...
        if report_no == 'a':
            ...
            # Display the ggbs content
            ggbs_txt = f'{mix_design_results["ggbs"][self.result_accuracy_switch]}'
            canvas.create_text(267.5,
                               495,
                               fill=white_color,
                               anchor='center',
    ...
    def display_report_a(self, canvas):
        ...
        # Display ggbs____ kg
        self.display_ggbs_quantity(report_no='a', canvas=canvas)
    ...
    def config_bv_display(self, canvas, batch_design_data: dict):
        ...
        # Adjust the batch volume of the ggbs content quantity
        ggbs_new_txt = f'{batch_design_data["ggbs"][self.result_accuracy_switch]}'
        canvas.itemconfigure(tagOrId='ggbs_qty_b', text=ggbs_new_txt)
    ...
    def adjust_batch_volume(self, canvas):
        ...
        if all(fill_status):
            ...
            self.analyzer.batch_to_desired_volume(mode='GGBS', mix_design_data=self.analyzer.design_results)
    ...
    def display_trial_mix_report(self, canvas):
        ...
        # Display ggbs___ kg
        self.display_ggbs_quantity(report_no='b', canvas=canvas)
    ...
    def display_final_results(self):
        ...
        # Top level window configuration
        self.title("OptiMix: Final Design Results (Portland Cement/Ground Granulated Blast-furnace Slag Concrete Mix)")
