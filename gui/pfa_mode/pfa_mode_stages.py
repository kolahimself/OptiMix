...
pfa/cement concrete mix design stages.
...
class StageOne(tk.Frame):
    ...
    def special_check(self):
        ...
        self.analyzer.special_check(mode='PFA')
    ...
    def calculate(self):
        ...
        self.analyzer.calculate_margin(mode='PFA')
        self.analyzer.calculate_target_mean_strength(mode='PFA')
        self.analyzer.calculate_approx_strength(mode='PFA')
        self.analyzer.calculate_fwc_ratio(mode='PFA')
...
class StageTwo(tk.Frame):
    ...
    def __init__(self, master_sii, analyzer, controller):
        ...
        # Make a list of all entries, color change and perform numerical check
        self.entry_list = [self.pfa_prop, self.cef]
    ...
    def display_fimages(self):
        ...
        # pfa proportion
        pfa_preloaded = load_tk_image(optimix_paths.pfa_assets['pfa_prop_pl'])
        self.stage_canvas.pfa_pl = pfa_preloaded
        self.stage_canvas.create_image(
            588,
            450,
            image=pfa_preloaded
        ...
        cef = load_tk_image(optimix_paths.pfa_assets['cef_image'])
        self.stage_canvas.cef_image = cef
        self.stage_canvas.create_image(
            688,
            478,
            image=cef
    ...
    def display_entries(self):
        """
        Handles display and data storage in tk entry widgets
        """

        # Entry 1 (pfa proportion in a cement-pfa mix)
        # Return previously saved values
        pfa_prop_value = tk.StringVar(self.stage_canvas)
        pfa_prop_value.set(self.analyzer.data['Specified variables']['pfa Proportion'])

        # Display entry 1
        self.pfa_prop = tk.Entry(
            self.stage_canvas,
            textvariable=pfa_prop_value,
            background=BG,
            foreground=white_color,
            font=lato,
            highlightthickness=1
        ...
        # Entry 2 (Cementing efficiency factor, k)
        # Return previously saved values
        k_value = tk.StringVar(self.stage_canvas)
        k_value.set(self.analyzer.data['Additional info']['Cementing Efficiency Factor'])
    ...
    def collect_data(self):
        ...
        # pfa proportion
        self.analyzer.sync_input(self.pfa_prop, 'Specified variables', 'pfa Proportion')

        # cementing efficiency factor, k
        self.analyzer.sync_input(self.cef, 'Additional info', 'Cementing Efficiency Factor')

    def set_inputs_to_default(self):
        """Sets all text entries and selection to default"""
        # Set all entries to their default values
        [entry.delete(0, tk.END) for entry in self.entry_list]
        self.entry_list[1].insert(0, "0.3")
    ...
    def check_fill_status(self):
        ...
        fill_status = []
        messagebox_has_been_shown = False
        pfa_prop_val = self.pfa_prop.get().strip()
        ...
        if all(element == 0 for element in fill_status):
        ...
        elif not pfa_prop_val or (pfa_prop_val and not pfa_prop_val.replace('.', '').isdigit()):
            ...
            if messagebox_has_been_shown:
                ...
                messagebox.showerror(
                    title="Invalid input",
                    message="Please input a valid value for the required pfa proportion.",
                    parent=self.stage_canvas
                )
    ...
    def calculate(self):
        ...
        Also given that the compulsory entries have been filled
        """
        self.analyzer.calculate_fw_content(mode='PFA')


class StageThree(tk.Frame):
    """Displays all widgets for the mix design in stage three
        - collects the input data
        - processes the input data for design and report viewing
    ...
    def display_fimages(self):
        ...
        limit_note = load_tk_image(optimix_paths.pfa_assets['lw'])
    ...
    def check_fill_status(self):
        ...
        self.collect_data()
        self.analyzer.calculate_cement_content(mode='PFA')
...
class StageFive(tk.Frame):
    ...
    def check_fill_status(self):
        ...
        # Perform background calculations before proceeding to check
        self.collect_data()
        self.analyzer.compute_fine_agg_proportion(mode='PFA')
        self.analyzer.compute_agg_content(mode='PFA')
    ...
    def calculate(self):
        ...
        self.analyzer.oven_dry_batching()
        self.analyzer.proportion_coarse_agg()
        self.analyzer.summarize_results(mode='PFA')
