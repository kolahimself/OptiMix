...
This module displays all stages and handles the data collection, processing and transition in the air entrained concrete
mix design stages
"""
...
class StageOne(tk.Frame):
    ...
    def __init__(self, master_si, analyzer, controller):
        ...
        self.entry_list = [self.x_strength, self.days, self.defective_rate, self.air_content,
                           self.ps_loss, self.sd, self.spec_k, self.s_margin, self.max_fc_ratio]
        self.set_active()
    ...
    def display_fimages(self):
        ...
        req_fimages = load_tk_image(optimix_paths.aem_assets['required_entries_image'])
    ...
    def display_entries(self):
        ...
        # Entry 4 (Air content)
        # Return previously saved values
        air_content_value = tk.StringVar(self.stage_canvas)
        air_content_value.set(self.analyzer.data['Specified variables']['Air Content'])

        self.air_content = tk.Entry(
            self.stage_canvas,
            textvariable=air_content_value,
            background=BG,
            foreground=white_color,
            font=lato,
            highlightthickness=1
        )
        self.air_content.place(
            x=385.0,
            y=300.0,
            width=77.0,
            height=28.0
        ...
        # Entry 5 (Percent strength Loss)
        # Return previously saved values
        ps_loss_value = tk.StringVar(self.stage_canvas)
        ps_loss_value.set(self.analyzer.data['Specified variables']['Strength Loss'])

        self.ps_loss = tk.Entry(
            self.stage_canvas,
            textvariable=ps_loss_value,
            background=BG,
            foreground=white_color,
            font=lato,
            highlightthickness=1
        ...
        # Entry 9 (Maximum free water-cement ratio)
        # Return previously saved values
    ...
    def collect_data(self):
        ...
        # Air content
        self.analyzer.sync_input(self.air_content, 'Specified variables', 'Air Content')

        # Strength loss
        self.analyzer.sync_input(self.ps_loss, 'Specified variables', 'Strength Loss')
    ...
    def special_check(self):
        ...
        self.analyzer.special_check(mode='AEM')
    ...
    def check_fill_status(self):
        ...
        for entry in [self.x_strength, self.days, self.air_content]:
        ...
        for entry in [text_box for text_box in self.entry_list if text_box not in [self.x_strength, self.days, self.air_content]]:
            value = entry.get().strip()
    ...
    def calculate(self):
        ...
        self.analyzer.calculate_margin(mode='AEM')
        self.analyzer.calculate_target_mean_strength(mode='AEM')
        self.analyzer.calculate_approx_strength(mode='AEM')
        self.analyzer.calculate_fwc_ratio(mode='AEM')
...
class StageTwo(tk.Frame):
    ...
    def calculate(self):
        ...
        Also given that the compulsory entries have been filled
        """
        self.analyzer.calculate_fw_content(mode='AEM')
...
class StageThree(tk.Frame):
    ...
    def check_fill_status(self):
        ...
        self.collect_data()
        self.analyzer.calculate_cement_content(mode='AEM')
    ...
    def calculate(self):
        """
        Performs mix design calculations on the mix design data class
        """
        self.analyzer.calculate_cement_content(mode='AEM')
...
class StageFour(tk.Frame):
    ...
    def check_fill_status(self):
        ...
        if self.analyzer.ssd_value_error or self.analyzer.invalid_ssd:
            ...
            self.analyzer.compute_wet_conc_density(mode='AEM')
            self.analyzer.override_density()
            self.analyzer.compute_total_agg_content(mode='AEM')
...
class StageFive(tk.Frame):
    ...
    def __init__(self, master_sv, analyzer, controller):
        ...
        self.entry_list = [self.pp, self.cagg_abs, self.fagg_abs, self.fr]
        self.set_active()
    ...
    def display_fimages(self):
        ...
        # Display the stage instructions
        instruction_v = load_tk_image(optimix_paths.aem_assets['instructions_v_aem'])
        ...
        fagg_reduc = load_tk_image(optimix_paths.aem_assets['fine_agg_reduc_image'])
        self.stage_canvas.fagg_reduc = fagg_reduc
        self.stage_canvas.create_image(
            489.5,
            235,
            image=fagg_reduc
    ...
    def display_entries(self):
        ...
        # Display entry 2
        # Return previously saved values (5% by default)
        fagg_reduc_value = tk.StringVar(self.stage_canvas)
        fagg_reduc_value.set(self.analyzer.data['Additional info']['Fine Aggregate Reduction'])
        self.fr = tk.Entry(
            self.stage_canvas,
            textvariable=fagg_reduc_value,
            background=BG,
            foreground=white_color,
            font=lato,
            highlightthickness=1
        ...
        # Entry 3 Fine aggregate absorption
        # Return previously saved values
        ...
        # Entry 4 Coarse aggregate absorption
        # Return previously saved values
    ...
    def collect_data(self):
        ...
        self.analyzer.sync_input(self.fr, 'Additional info', 'Fine Aggregate Reduction')
    ...
    def check_fill_status(self):
        ...
        # Perform background calculations before proceeding to check
        self.collect_data()
        self.analyzer.compute_fine_agg_proportion(mode='AEM')
        self.analyzer.compute_agg_content(mode='AEM')
    ...
    def calculate(self):
        """
        Performs mix design calculations on the mix design data class (AEM mix design mode)
        Also given that the compulsory entries have been filled
        """
        self.analyzer.compute_fine_agg_proportion(mode='AEM')
        self.analyzer.compute_agg_content(mode='AEM')
        self.analyzer.oven_dry_batching()
        self.analyzer.proportion_coarse_agg()
        self.analyzer.summarize_results(mode='AEM')
